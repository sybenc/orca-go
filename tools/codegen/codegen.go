...
func (g *Generator) generate(typeName string) {
	...
	for _, v := range values {
		...
		//fmt.Println(v.originalName, code, description)
		g.Printf("\tregister(%s, %s, \"%s\")\n", v.originalName, code, description)
	}
	g.Printf("}\n")
}

func (g *Generator) generateI18nJson(typeName string) {
	values := make([]Value, 0, 100)
	for _, file := range g.pkg.files {
		// Set the state for this run of the walker.
		file.typeName = typeName
	...
	for _, v := range values {
		...
		codeMap[v.originalName] = description
	}
	...
	encoder.SetIndent("", "  ")
	if err := encoder.Encode(codeMap); err != nil {
...
func (v *Value) ParseComment() (string, string) {
	reg := regexp.MustCompile(`\w\s*-\s*(\d{3})\s*:\s*(\p{Han}+.*)\s*[.。]\n*`)
